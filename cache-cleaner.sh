...
# НЕБОЛЬШИЕ НАСТРОЙКИ
# Очистка будет каждые Х дней. Цыфру можно поменять
# Дни(d) можно поменять на чясы(h) или даже минуты(m) или секунды
delay="15d"
...
main(){
#   Цыкл while будет выполнать следующее действие бесконечно:
# --  Ждать Х дней (15)
# --  Чистить кеш
# --  Создавать логи (если эта функцыя включена)
    while true; do
        sleep $delay
        clean_cache
        if [[ logs -eq 1 ]]; then
            # [КОГДА] [КТО] "Очистел кеш"
